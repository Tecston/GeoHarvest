...
export const useCalEmbed = (options: CalEmbedOptions) => {
  useEffect(() => {
    void (async () => {
      ...
      cal("ui", { styles: options.styles, hideEventTypeDetails: options.hideEventTypeDetails, layout: options.layout });
    })();
  }, [options]);
  return { namespace: options.namespace, layout: options.layout };
};
